...
void LEDDisplay::setConf(uint8_t m, uint8_t d){
   mode=m;
   direction=d;
}
...
void LEDDisplay::add(uint16_t w, uint8_t mode){
   ...
   current_pos++;
   if(current_pos>=PIXELCOUNT) current_pos=0;
   if(offset<0 && current_pos==last_run_pos) _is_done=true;
}

uint8_t LEDDisplay::add(char c, uint8_t mode) {
  if(gfxFont==NULL) return 0;
  if (c < (uint8_t)pgm_read_byte(&gfxFont->first)) return 0;
  if (c > (uint8_t)pgm_read_byte(&gfxFont->last)) return 0;
...
void LEDDisplay::setSpeed(void){
    wait=(micros()-last_rot)/360-16;
    if(wait*360>200000L) wait=200000L/360;
    last_rot=micros();
}
...
void LEDDisplay::run(void){
  ...
  if(direction==ANTICLOCKWISE){
    ...
    write(buffer+current_pos,PIXELCOUNT-current_pos,wait,true);  
 } else {
    write(buffer+current_pos,PIXELCOUNT-current_pos,wait,false);  
    write(buffer,current_pos,wait,false);
  }
 } else
   write(buffer,PIXELCOUNT,wait,direction==ANTICLOCKWISE);
}

void LEDDisplay::print(void){
  LEDArray::print(buffer,PIXELCOUNT);
}
...
void LEDDisplay::runningText(const char* text){
  ...
  if(((uint16_t) millis() - _last_shift)>_shift_wait){
    for(uint8_t i=0;i<_shift;i++){
       if(offset>=0){
	 if(text[offset]) add(text[offset]);
         else {
	   offset=-100;
...
void LEDDisplay::runningBitmapPROGMEM(const uint8_t* bitmap, uint16_t bitmap_length){
  _is_done=false;
  if(((uint16_t) millis() - _last_shift)>_shift_wait){
    for(uint8_t i=0;i<_shift;i++){
      uint16_t v=0;
	  ...
	  *(((uint8_t*)&v)+1)=pgm_read_byte(bitmap+offset+bitmap_length/2);
